...
def select_model(model_name, config):
    ...
    if model_name == 'mbcnn':
        ...
        model = sm.Unet(input_shape=(None, None, c), classes=n_outputs, activation='softmax',  encoder_weights=None)
    elif model_name == 'fpn':
        model = sm.FPN(input_shape=(None, None, c), classes=n_outputs, activation='softmax', encoder_weights=None, backbone_name='resnet34')
