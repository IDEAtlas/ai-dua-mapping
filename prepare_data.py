...
from preprocessing import adm_boundaries, stac_api, fetch_ghsl, create_ref, extract_patch, create_density, google_buildings
...
if caller == "classify":
    ...
    # Format: {city_normalized}_reference_data_{year}_v*.tif
    existing_ref_tif = glob.glob(os.path.join(output_dir, f"reference_data/{city_normalized}_reference_data_{year}_v*.tif"))
    
    if existing_ref_tif:
        output_reference = existing_ref_tif[0]
    else:
        # Use v1 as default output version
        output_reference = os.path.join(output_dir, f"reference_data/{city_normalized}_reference_data_{year}_v1.tif")
        
        # Look for reference GeoJSON input (any version)
        # Format: {city_normalized}_reference_data_{year}_v*.geojson
        ref_geojson_files = glob.glob(os.path.join(output_dir, "reference_data/", f"{city_normalized}_reference_data_{year}_v*.geojson"))
        
        if ref_geojson_files:
            path_duas = ref_geojson_files[0]
        else:
            # Try without version suffix
            ref_geojson_files = glob.glob(os.path.join(output_dir, "reference_data/", f"{city_normalized}_reference_data_{year}.geojson"))
            if ref_geojson_files:
                path_duas = ref_geojson_files[0]
            else:
                logger.error(f"No reference data file found. Expected: {city_normalized}_reference_data_{year}_v*.geojson")
                progress.close()
                exit(1)
